...
export class ReclamosComponent implements OnInit {
  ...
  formularioDetalleReclamos(){
    this.detallereclamosForm = this.formBuilder.group({
      id: [''],
      cantidad: ['',[Validators.required, Validators.pattern('[1-30]{1,30}')]],
      precio_pedido: ['',[Validators.required,Validators.pattern('[0-9]{2}.[0-9]{2}')]],
      precio_llegada: ['',[Validators.required,Validators.pattern('[0-9]{2}.[0-9]{2}')]],
      idreclamo: [''],
  ...
  postDataDetallePedidos = () => {
    ...
    if(this.detallereclamosForm.valid){
      this.http.post(environment.API_URL, register)
      .subscribe( data => { 
        returning = data
      })
